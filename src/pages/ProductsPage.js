    ...
    <div className="space-y-8">
      <section className="overflow-hidden rounded-[2rem] bg-gradient-to-r from-emerald-600 via-emerald-500 to-lime-400 p-8 text-white shadow-lg">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-50/90">
              Product Page
            </p>
            <h1 className="mt-3 max-w-3xl text-4xl font-black md:text-5xl">
              Belanja produk Toko Happy dengan nuansa marketplace yang cepat dan praktis.
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-emerald-50/90">
              Semua product ditarik langsung dari API. Member bisa menambahkan product ke keranjang
              langsung dari grid belanja.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <div className="rounded-3xl bg-white/20 px-5 py-4 backdrop-blur">
              <p className="text-xs uppercase tracking-[0.2em] text-emerald-100">Total Produk</p>
              <p className="mt-2 text-3xl font-black">{storefrontProducts.length || "--"}</p>
            </div>
            <div className="rounded-3xl bg-slate-950/20 px-5 py-4 backdrop-blur">
              <p className="text-xs uppercase tracking-[0.2em] text-emerald-100">Keranjang Member</p>
              <p className="mt-2 text-3xl font-black">{memberCartCount}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-200 md:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Explore product terbaru dengan layout grid ala e-commerce.
            </p>
          </div>
          <button
            ...
            onClick={loadStorefrontProducts}
            type="button"
          >
            Refresh Product
          </button>
        ...
        {storefrontError ? (
          <p className="mt-5 rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-600">{storefrontError}</p>
        ) : null}
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
            {storefrontProducts.map((product) => {
              ...
              return (
                <article
                  key={product.id}
                  className="group overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white transition hover:-translate-y-1 hover:border-emerald-300 hover:shadow-lg"
                >
                  ...
                  <div className="space-y-3 p-4">
                    <h3 className="line-clamp-2 min-h-[3.5rem] text-sm font-semibold leading-6 text-slate-900">
                      {product.title}
                    </h3>
                    <p className="line-clamp-2 min-h-[2.8rem] text-xs leading-5 text-slate-500">
                      {product.description}
                    </p>
                    ...
                    <button
                      ...
                      onClick={() => handleMemberAddProduct(product)}
                      type="button"
                    >
